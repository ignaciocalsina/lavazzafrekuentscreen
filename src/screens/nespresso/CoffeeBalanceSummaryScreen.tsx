...
const CoffeeBalanceSummaryScreen = () => {
    ...
    <div className="screen-enter relative flex-1 overflow-hidden bg-nes-coffee flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-6">
        <div className="w-[80%] rounded-xl bg-white/8 border border-white/15 p-3 text-white text-[13px] backdrop-blur-sm">
          ...
          <button
            onClick={() => navigate(fullyCovered ? 'coffee_processing' : 'coffee_balance_pay')}
            className="mt-2 w-full bg-nes-gold text-nes-cream rounded-full py-2 font-serif-nes text-[13px] tracking-[0.15em] active:scale-[0.98] transition-transform shadow"
          >
            {fullyCovered ? 'CONFIRMAR' : 'PROCEDER AL PAGO'}
          </button>
        </div>
      </div>
    </div>
...
export default CoffeeBalanceSummaryScreen;
