...
const CoffeeBundlePayScreen = () => {
  const { navigate, setPaymentMethod, bundleType } = useApp();
  ...
  const caption = (
    <>
      {isWeek ? 'Bono semanal' : 'Bono mensual'} · {coffees} cafés ·{' '}
      <span className="font-semibold">{fmt(total)}</span>
      <span className="block text-[10px] opacity-80 italic">Paga {paid}, llévate {coffees}.</span>
    </>
  );
